...
# Load 2023 data
df2023 = pd.read_excel('data/data_filippoi_2023.xlsx', sheet_name='DATA2023')
df2023.columns = ['date', 'time', 'flow1', 'total_flow', 'hourly_flow', 'daily_flow', 'monthly_flow']
# Combine 'date' and 'time' into a single datetime column
df2023['datetime'] = pd.to_datetime(df2023['date'].astype(str) + ' ' + df2023['time'].astype(str), errors='coerce')
df2023.set_index('datetime', inplace=True)
df2023['hourly_flow'] = pd.to_numeric(df2023['hourly_flow'], errors='coerce')
# Keep only 'hourly_flow' column (with datetime index) and drop rows with missing values
df2023 = df2023[['hourly_flow']].dropna()
# Load 2024 data
df2024 = pd.read_excel('data/data_filippoi_2024.xlsx', sheet_name='DATA2024')
df2024.columns = ['datetime', 'flow', 'flow_calibration', 'total_flow', 'hourly_flow', 'daily_flow', 'monthly_flow']
df2024['datetime'] = pd.to_datetime(df2024['datetime'].astype(str), errors='coerce')
df2024.set_index('datetime', inplace=True)
df2024['hourly_flow'] = pd.to_numeric(df2024['hourly_flow'], errors='coerce')
# Keep only 'hourly_flow' column (with datetime index) and drop rows with missing values
df2024 = df2024[['hourly_flow']].dropna()
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.015, shuffle=False
)
